...
class PilotDenyCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'pilot:deny 
     ...
     * @var string
     */
    protected $description = 'Deny permissions to a user through a role';
    ...
    public function handle()
    {
        ...
        if(!PilotRole::contains($user)) {
            $this->error(sprintf('User %s does not have the %s role', $argUser, $argRole));
            return;
        }
        $role->users()->detach($user);
        $this->info(sprintf('Denied %s permissions to user %s', $argRole, $argUser));
    }
}
